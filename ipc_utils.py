...
def pack_ipc_payload(
    handle: bytes, total_size: int, num_envs: int, state_dim: int, action_dim: int
) -> bytes:
    """
    ...
    if len(handle) != CUDA_IPC_HANDLE_SIZE:
        raise ValueError(
            f"IPC handle 大小错误: 期望 {CUDA_IPC_HANDLE_SIZE}, 实际 {len(handle)}"
        )

    return struct.pack(
        PAYLOAD_FORMAT, handle, total_size, num_envs, state_dim, action_dim
    )
...
def unpack_ipc_payload(data: bytes) -> tuple:
    ...
    handle, total_size, num_envs, state_dim, action_dim = struct.unpack(
        PAYLOAD_FORMAT, data
    )
